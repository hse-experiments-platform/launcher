...
func initService(ctx context.Context, maker token.Maker) pb.LauncherServiceServer {
...
func runGRPC(ctx context.Context, c context.CancelFunc, server pb.LauncherServiceServer, grpcAddr string, maker token.Maker) {
	opts := []logging.Option{
...
func runHTTP(ctx context.Context, c context.CancelFunc, grpcAddr string) {
	...
	err := pb.RegisterLauncherServiceHandlerFromEndpoint(ctx, rmux, grpcAddr, opts)
	if err != nil {
